...
// Function to simulate CO2 level measurement
...
// Function to simulate Natural Gas level measurement
...
void* count_cars_entering(void* arg) {
    while (1) {
        ...
        sleep(rand()%3); //Time delay between 0 and 9 seconds
...
int main() {
    pthread_t thread1, thread2, monitor_thread, co2_thread, natural_gas_thread;

        // Initialize mutexes
    pthread_mutex_init(&cars_mutex, NULL);
    pthread_mutex_init(&co2_mutex, NULL);
    pthread_mutex_init(&natural_gas_mutex, NULL);
    pthread_mutex_init(&cars_exiting_mutex, NULL);
    ...
    pthread_mutex_destroy(&cars_exiting_mutex);
